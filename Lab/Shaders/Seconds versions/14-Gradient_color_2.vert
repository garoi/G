...
void main()
{
	...
	float lambda = ((gl_Position.y / gl_Position.w) - MINY)/(MAXY -MINY);
	lambda *= 4.0; 
	if (lambda > 4.0) gl_FrontColor = blue;
	else if (lambda <= 4.0 && lambda >3.0) gl_FrontColor  = mix(cyan, blue, fract(lambda));
